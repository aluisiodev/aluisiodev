...
def svg_overwrite(filename, age_data, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data):
    ...
    root = tree.getroot()
    # As larguras abaixo mantem todas as linhas com exatamente 72 colunas.
    # 'contrib_data' e 'loc_add' nao tem pontilhado proprio, entao o tamanho
    # deles e descontado das larguras vizinhas para a borda nao dancar.
    tam_contrib = len('{:,}'.format(contrib_data))
    tam_add = len(str(loc_data[0]))

    justify_format(root, 'age_data', age_data, 61)
    justify_format(root, 'commit_data', commit_data, 28)
    justify_format(root, 'follower_data', follower_data, 17)
    justify_format(root, 'repo_data', repo_data, 16)
    justify_format(root, 'contrib_data', contrib_data)
    justify_format(root, 'star_data', star_data, 19 - tam_contrib)
    justify_format(root, 'loc_data', loc_data[2], 20)
    justify_format(root, 'loc_add', loc_data[0])
    justify_format(root, 'loc_del', loc_data[1], 21 - tam_add)
...
if __name__ == '__main__':
    """
    Baseado no projeto de Andrew Grant (Andrew6rant), 2022-2025
    """
    ...
    age_data, age_time = perf_counter(daily_readme, datetime.datetime.strptime(BIRTHDAY, '%Y-%m-%d'))
